...
def set_button_position(im):
    ...
    # print('xxxxxxx',swipe_x1, swipe_y1, swipe_x2, swipe_y2)


def jump(distance):
    press_time = distance * press_coefficient
    press_time = press_time + 6 * random.random() #随机部分
    press_time = max(press_time, 200)   # 设置 200 ms 是最小的按压时间
    press_time = int(press_time)
    cmd = 'adb shell input swipe {x1} {y1} {x2} {y2} {duration}'.format(
        x1=swipe_x1+random.randint(0,100),
        y1=swipe_y1+random.randint(0,100),
        x2=swipe_x2+random.randint(0,100),
        y2=swipe_y2+random.randint(0,100),
        duration=press_time
    )
    print(cmd)
    os.system(cmd)
...
def main():
    ...
    while True:
        ...
        print(math.sqrt((board_x - piece_x) ** 2 + (board_y - piece_y) ** 2))
        save_debug_creenshot(ts, im, piece_x, piece_y, board_x, board_y)
        backup_screenshot(ts)
        time.sleep(1+random.random())   # 为了保证截图的时候应落稳了，多延迟一会儿
